...
import { AlertTriangle, CheckCircle2, TrendingDown, Shield, PiggyBank } from 'lucide-react';
import {
...
const Insights = () => {
  ...
  const { data: transactions = [], isLoading: transactionsLoading } = useQuery({ queryKey: ['transactions'], queryFn: getTransactions });

  if (insightsLoading || transactionsLoading) return <AppLayout><div className="p-8">Loading insights...</div></AppLayout>;

  const data = insightsData || { debtToIncomeRatio: 0, emergencyFundCoverage: 0, savingsRate: 0 };
