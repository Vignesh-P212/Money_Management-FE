...
const Goals = () => {
  ...
  const createGoalMutation = useMutation({ mutationFn: createGoal, onSuccess: () => queryClient.invalidateQueries({ queryKey: ['goals'] }) });
  const updateGoalMutation = useMutation({ mutationFn: ({ id, data }) => updateGoal(id, data), onSuccess: () => queryClient.invalidateQueries({ queryKey: ['goals'] }) });
  const deleteGoalMutation = useMutation({ mutationFn: deleteGoal, onSuccess: () => queryClient.invalidateQueries({ queryKey: ['goals'] }) });
  ...
  const openEdit = (g) => {
    setEditId(g._id);
    setForm({ title: g.title, targetAmount: String(g.targetAmount), currentAmount: String(g.currentAmount), deadline: g.deadline?.slice(0, 10) || '' });
    setDialogOpen(true);
  };
  ...
  const saveGoal = () => {
    ...
    if (editId) {
      updateGoalMutation.mutate({ id: editId, data: goalData });
    } else {
  ...
  if (isLoading) return <AppLayout><div className="p-8">Loading goals...</div></AppLayout>;
            ...
            <div key={g._id} className="bg-card rounded-xl border border-border p-5 animate-fade-in">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex gap-1">
                  ...
                  <button onClick={() => deleteGoalMutation.mutate(g._id)} className="p-1.5 text-muted-foreground hover:text-destructive transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </div>
